...
const UpdateGroup = () => {
  ...
  const fetchGroupDetails = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://api:5000/group/getgroup/${groupId}`, {
        method: 'GET',
        credentials: 'include',
      });
      ...
      setSelectedMembersList(data.group.members.map(member => member._id));
    } catch (error) {
  ...
  const fetchUsers = async () => {
    try {
      ...
      setMembers([...filteredUsers, currentUserData]);

      if (currentUserData) {
        setSelectedMembersList(prevList => [...prevList, currentUserData._id]);
      }
    } catch (error) {
  ...
  const handleUpdateGroup = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`http://api:5000/group/updategroup/${groupId}`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <div className='container mx-auto p-4 max-w-4xl bg-white text-black shadow-md rounded-lg'>
      ...
      <form onSubmit={handleUpdateGroup} className="flex flex-wrap -mx-2">
